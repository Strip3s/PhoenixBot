...
print(
    normal_color
    + "Welcome To Phoenix Bot - In times of crisis, the wise build bridges while the foolish build barriers."
)
...
def send_webhook(webhook_type, site, profile, task_id, image_url):
    if settings.webhook != "":
        ...
        embed.set_footer(
            text="Via Phoenix Bot", icon_url="https://i.imgur.com/60G42xE.png"
        )
...
def change_driver(status_signal, loc):
    ...
    val = (
        ...
        + "_"
        + "".join(random.choices(string.ascii_letters + string.digits, k=22))
        + "_"
    )
