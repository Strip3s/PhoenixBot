...
class Walmart:
    def __init__(
        ...
        if settings.dont_buy:
            starting_msg = (
                "Starting in dev mode will not actually checkout (dont_buy = True)"
            )
                ...
                if r.status_code == 200:
                    ...
                    if self.is_captcha(r.text):
                        self.status_signal.emit(
                            {"msg": "CAPTCHA - Opening Product Page", "status": "error"}
                        )
    ...
    def atc(self, offer_id):
        ...
        while True:
            ...
            try:
                ...
                if self.is_captcha(r.text):
                    self.status_signal.emit(
                        {"msg": "Opening CAPTCHA", "status": "error"}
                    )
                    self.handle_captcha(self.product)
                    return False
                ...
                if (
                    ...
                    and json.loads(r.text)["checkoutable"] == True
                ):
                    self.status_signal.emit(
    ...
    def submit_order(self):
        ...
        if settings.dont_buy is True:
            ...
            self.handle_captcha(
                "https://www.walmart.com/checkout/#/payment",
                close_window_after=False,
                redirect=True,
            )  # OPEN BROWSER TO SEE IF SHIT WORKED
            self.check_browser()
            return
    ...
    def handle_captcha(
        self, url_to_open, close_window_after=True, redirect=False
        ...
        )  # removes SSL errors from terminal
        options.add_experimental_option(
            "excludeSwitches", ["enable-logging"]
        )  # removes device adapter errors from terminal
