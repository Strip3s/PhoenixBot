...
class SettingsPage(QtWidgets.QWidget):
    ...
    def create_font(self, family, pt_size) -> QtGui.QFont:
        ...
        font.setPointSize(
            pt_size
        ) if platform.system() == "Darwin" else font.setPointSize(pt_size * 0.75)
    ...
    def setupUi(self, settingspage):
        ...
        self.webhook_edit = self.create_edit(
            self.settings_card,
            QtCore.QRect(30, 50, 411, 21),
            self.small_font,
            "Webhook Link",
        )
        self.webhook_header = self.create_header(
            self.settings_card,
            QtCore.QRect(20, 10, 101, 31),
            self.header_font,
            "Webhook",
        )
        ...
        self.browser_checkbox = self.create_checkbox(
            QtCore.QRect(30, 90, 300, 20), "Browser Opened"
        )
        self.order_checkbox = self.create_checkbox(
            QtCore.QRect(30, 120, 221, 20), "Order Placed"
        )
        self.paymentfailed_checkbox = self.create_checkbox(
            QtCore.QRect(30, 150, 121, 20), "Payment Failed"
        )

        self.general_header = self.create_header(
            self.settings_card,
            QtCore.QRect(20, 180, 101, 31),
            self.header_font,
            "General",
        )
        self.onfailed_checkbox = self.create_checkbox(
            QtCore.QRect(30, 220, 221, 20), "Open browser on payment failed"
        )
        self.buy_one_checkbox = self.create_checkbox(
            QtCore.QRect(30, 250, 221, 20), "Stop All after success"
        )
        self.dont_buy_checkbox = self.create_checkbox(
            QtCore.QRect(30, 280, 400, 20),
            "Don't actually buy items. (Used for dev and testing)",
        )
        self.random_delay_start = self.create_edit(
            self.settings_card,
            QtCore.QRect(30, 310, 235, 20),
            self.small_font,
            "Random Start Delay (Default is 10ms)",
        )
        self.random_delay_stop = self.create_edit(
            self.settings_card,
            QtCore.QRect(30, 335, 235, 20),
            self.small_font,
            "Random Stop Delay (Default is 40ms)",
        )
        self.proxies_header = self.create_header(
            self.settingspage,
            QtCore.QRect(30, 10, 81, 31),
            self.create_font("Arial", 22),
            "Settings",
        )
        self.target_user_edit = self.create_edit(
            self.settings_card,
            QtCore.QRect(30, 365, 235, 20),
            self.small_font,
            "Target.com Username (Email/Cell #)",
        )
        self.target_pass_edit = self.create_edit(
            self.settings_card,
            QtCore.QRect(30, 390, 235, 20),
            self.small_font,
            "Target.com Password)",
        )
        self.gamestop_user_edit = self.create_edit(
            self.settings_card,
            QtCore.QRect(300, 365, 235, 20),
            self.small_font,
            "Gamestop.com Username (Email)",
        )
        self.gamestop_pass_edit = self.create_edit(
            self.settings_card,
            QtCore.QRect(300, 390, 235, 20),
            self.small_font,
            "Gamestop.com Password)",
        )
    ...
    def set_data(self):
        ...
        try:
            self.target_pass_edit.setText(
                (Encryption().decrypt(settings["target_pass"].encode("utf-8"))).decode(
                    "utf-8"
                )
            )
        except:
        ...
        try:
            self.gamestop_pass_edit.setText(
                (
                    Encryption().decrypt(settings["gamestop_pass"].encode("utf-8"))
                ).decode("utf-8")
            )
        except:
            self.gamestop_pass_edit.setText("")
    ...
    def save_settings(self):
        settings = {
            ...
            "target_pass": Encryption()
            .encrypt(self.target_pass_edit.text())
            .decode("utf-8"),
            "gamestop_user": self.gamestop_user_edit.text(),
            "gamestop_pass": Encryption()
            .encrypt(self.gamestop_pass_edit.text())
            .decode("utf-8"),
        }
    ...
    def update_settings(self, settings_data):
        ...
        if settings_data.get("gamestop_pass", "") != "":
            settings.gamestop_pass = (
                Encryption().decrypt(settings_data["gamestop_pass"].encode("utf-8"))
            ).decode("utf-8")
